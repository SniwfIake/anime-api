...
export default async function handler(request) {
    ...
    try {
        if (path === "/api/search" || path === "/api") {
            const query = url.searchParams.get('q');
            if (!query) return new Response(JSON.stringify({ error: "Missing query parameter 'q'" }), { status: 400, headers });
        ...
        if (path === "/api/watch") {
            ...
            return new Response(JSON.stringify({ embed: streamUrl }), { headers });
        }
        
        return new Response(JSON.stringify({ message: "Vercel API Online. Endpoints: /api/search, /api/info, /api/watch" }), { headers });
